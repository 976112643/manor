...
class ShopController extends AdminController {
	...
	protected $limit=15;
	protected $cache_name='shop_cache';		/* 缓存名 */
...
// 		$map['shop_status'] = array('eq','1');
		$map['shop_status'] = array(in,array(1,2,3));
		$map['member.username'] = array('neq','');
// 		$map['title'] = array('neq','');
...
// 			$good_at = substr($str,0,-1);/*将擅长组装成字符串*/
	...
	public function undergo(){
		if(IS_POST){
			...
			if(is_numeric($result)){
				$this->success("修改成功！！",U('Backend/Shop/Shop/undergo',array("id"=>$shop_id)));
			}else{
				$this->error('修改失败！！',U('Backend/Shop/Shop/undergo',array("id"=>$shop_id)));
			}
		}else{
			$gets = I('get.');
			$id = $gets['id'];
	...
	public function shop_success(){
		if(IS_POST){
			...
			$result = update_data('shop_undergo');
			if(is_numeric($result)){
				$this->success("修改成功！！",U('Backend/Shop/Shop/shop_success',array("id"=>$shop_id)));
			}else{
				$this->error('修改失败！！',U('Backend/Shop/Shop/shop_success',array("id"=>$shop_id)));
			}
		}else{
			$gets = I('get.');
			$id = $gets['id'];
