...
class ThemeController extends AdminController {
	...
	protected $cache_name = 'themes_cache';
	/*
	*用户列表
	*	需求分析
	*		需要将所有用户的信息全部显示到这张表中
	...
	*/
	public function index(){
		...
		if($keywords){
			...
			$map['title|price']=array('like', "%$keywords%");//这里的username是真实姓名还是平台生成的呢？？这里还可以添加其他的关键词
	...
	public function update(){
		if(IS_POST){
			...
			if(is_numeric($result)){
				...
				/*将皮肤缩略图更新到数据库*/
				multi_file_upload(I('thumb'),'Uploads/Shop','theme','id',$result,'thumb');
