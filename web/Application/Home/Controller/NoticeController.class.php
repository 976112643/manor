...
class NoticeController extends HomeController {
	...
	**/
    public function index(){
		//将type为news的数据查询出来
		$map['status'] = array('GT',-1);//将正常的公告查询出来
		$map['type'] = 'notice';//将新闻查询出来，而不是别的
		...
		$map_notice['type']	= 'news';
		$map_notice['recommend'] =	1;//被推荐的新闻
		//查询出来
		...
		$data['news']	=	$news_result;//这个表示公告
		$data['notice'] = $notice_result;//这个表示新闻
		$this->assign($data);
        $this->display();
    }
