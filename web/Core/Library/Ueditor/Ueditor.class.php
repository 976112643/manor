...
class Ueditor{
	...
	private $rootpath = '/Uploads/UserFile';
	
	public function __construct($uid = ''){
		//uid 为空则导入当前会话uid
		
		\Ueditor\FileStorage::connect(STORAGE_TYPE);
		//导入设置
	...
	private function uploadFile($config,$fieldName){
		...
		if(!$info){
			...
			$data = array(
				'state'=>"SUCCESS",
				'url'=>\Ueditor\FileStorage::getPath($rootpath,$info['savepath'].$info['savename']),
				'title'=>$info['savename'],
				'original'=>$info['name'],
				'type'=>'.' . $info['ext'],
				'size'=>$info['size'],
	...
	private function uploadBase64($config,$fieldName){
		...
		if(\Ueditor\FileStorage::put($rootpath,$filename,$img)){
			$data=array(
				'state'=>'SUCCESS',
				'url'=>\Ueditor\FileStorage::getPath($rootpath,$filename),
				'title'=>$imgname,
				'original'=>'scrawl.png',
				'type'=>'.png',
				'size'=>strlen($img),
	...
	private function listFile($config){
		...
		$path = $config['path'];
		$path = str_replace("{uid}",session("member_id"), $path);
		$files = \Ueditor\FileStorage::listFile($rootpath,$path, $allowFiles);
		//return $files;
		if (!count($files)) {
	...
	private function getFullPath($path){
		...
		$format = str_replace("{uid}",session("member_id"), $format);
